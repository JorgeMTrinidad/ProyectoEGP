...
class IngresoController extends Controller
{
            ...
            $ingresos = Ingreso::join('proveedores', 'ingresos.idproveedor', '=', 'proveedores.id')
                ...
                ->where(function ($query) {
                    if (session('user_roll') == 1) {
                        $query->where('detalle_ingresos.revision', '<>', 'INCORRECTO');
                        $query->where('detalle_ingresos.revision', '=', 'CORRECTO');
                    }
                })
    ...
    public function show($id)
    {
        ...
        $detalles = DetalleIngreso::join('productos', 'detalle_ingresos.idproducto', '=', 'productos.id')
            ->select('detalle_ingresos.id', 'detalle_ingresos.cantidad', 'detalle_ingresos.revision', 'detalle_ingresos.precio', 'productos.nombre as producto')
    ...
    {
        // dd($request->revision);
        $ingreso = DetalleIngreso::where('id', $id)->update(['revision' => $request->revision]);
        // return Redirect::to('ingreso');
